<div class="research-guide-form">
	<form action="https://discovery.nationalarchives.gov.uk/results/r" id="dolSearch" method="get" name="searchform5532" role="search" aria-label="Search the records">
		<fieldset>
			<div class="field-row">
				<label for="firstname">First name</label>
				<input id="firstname" class="inlineSearch" name="_fn">
			</div>
			<div class="field-row">
				<label for="lastname">Last name</label>
				<input id="lastname" class="inlineSearch" name="_ln">
			</div>
			<div class="field-row">
				<label for="rank">Rank</label>
				<input id="rank" class="inlineSearch" name="_ttl">
			</div>
			<div class="field-row">
				<label for="servicenumber">Service number</label>
				<input id="servicenumber" class="inlineSearch" name="_no">
			</div>
			<div class="field-row">
				<label for="unit">Unit</label>
				<input id="unit" class="inlineSearch" name="_crp">
			</div>
			<div class="field-row">
				<label for"medal"="">Medal</label>
				<input id="medal" class="inlineSearch" name="_occ">
			</div>
		</fieldset>
		<fieldset>
			<legend>Date (YYYY):</legend>
			<div class="field-row">
				<label for="fromDate">From</label>
				<input id="fromDate" class="inlineSearch" name="_sd">
			</div>
			<div class="field-row">
				<label for="toDate">To</label>
				<input id="toDate" class="inlineSearch" name="_ed">
			</div>
		</fieldset>
		<div class="button-holder">
			<input class="submit" type="submit" value="Search">
		</div>
		<input name="discoveryCustomSearch" value="true" type="hidden">
		<input id="Hidden1" name="_cr1" value="WO 373" type="hidden">
		<input id="Hidden2" name="_col" value="200" type="hidden">
		<input id="Hidden3" name="_dt" value="H" type="hidden">
		<input id="Hidden4" name="_hb" value="tna" type="hidden">
	</form>
</div>